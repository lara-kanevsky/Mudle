...
app.post("/login", async function (request, response) {
  ...
  if (!mail || !password) {
    ...
    let userFound = await AuthenticationLogic.userCredentialsAreValid(
      mail,
      password
    );
...
apiProtectedRouter.post("/items", async (req, res) => {
  let result = await new ItemsLogic().insertNewItem(
    req.decodedToken.id,
    req.body,
    "duenio"
  );
...
apiProtectedRouter.post("/items/compartir", async (req, res) => {
  ...
  if (!idItem || !tipoPermiso || !arrayMailsUsuarios) {
    res
      .status(400)
      .send(
        "Los campos son: idItem, tipoPermiso ('read','read-write','duenio'), arrayMailsUsuarios. Alguno esta faltando."
      );
  }

  let result = await new ItemsLogic().compartirItemConMuchos(
    idItem,
    tipoPermiso,
    arrayMailsUsuarios
  );
